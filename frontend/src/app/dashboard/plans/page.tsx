...
───────────────────────────────────────────── */
function CalorieDonut({ consumed, total }: { consumed: number; total: number }) {
  ...
  const slices = isOver
    ...
    : [
        { name: 'Consumed',  value: consumed,  fill: 'var(--color-brand-purple)' },
        { name: 'Remaining', value: remaining, fill: 'var(--color-brand-pale)' },
      ]
  ...
  return (
    <div className="relative flex h-[184px] w-[184px] items-center justify-center flex-shrink-0">
      ...
      <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center text-center">
        <p className="text-[15px] font-bold leading-tight text-brand-slate">{kcalLabel}</p>
        <p className="text-[10px] text-brand-purple">{totalLabel}</p>
        <p className="mt-1 text-[12px] font-semibold text-brand-slate">{remLabel}</p>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[auto_1fr]">
        ...
        <Card className="flex flex-col items-center gap-6 p-6 shadow-lg sm:flex-row sm:items-start">
          ...
          <div className="flex flex-col justify-center gap-3 text-sm">
            {[
              { dotClass: 'bg-brand-purple', label: 'Consumed',  extra: `${consumed.toLocaleString()} Kcal` },
              { dotClass: 'bg-brand-pale border border-brand-mauve', label: 'Remaining', extra: `${Math.max(remaining, 0).toLocaleString()} Kcal` },
              { dotClass: 'bg-brand-gold', label: 'Over-Limit', extra: remaining < 0 ? `${Math.abs(remaining)} Kcal` : '-' },
            ].map((item) => (
  ...
  <div className="flex items-start gap-4">
    ...
    <div className="space-y-1.5">
      ...
      <p className="text-sm text-brand-slate/60">
        Try swapping white rice with quinoa for more fiber and nutrients.
      </p>
      <div className="pt-1">
        <Button
          variant="gold"
