...
export default function SignupPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccess(`Registration successful! Welcome ${userData.full_name}. Redirecting...`)
      ...
      <div className="w-full max-w-[23rem] rounded-2xl border border-brand-pale bg-white px-6 py-7 shadow-[0_20px_50px_-22px_#9567B9]">
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-xl font-bold text-brand-slate">Create Account</h1>
          </div>
        </div>

        <p className="mb-5 text-sm text-brand-slate/70">Set up your account and get started.</p>
        ...
        <p className="mt-5 text-center text-sm text-brand-slate/70">
          ...
          <Link href="/login" className="font-semibold text-brand-gold hover:text-brand-deep">
            Login
          </Link>
        </p>
      </div>
