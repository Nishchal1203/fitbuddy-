...
export default function GoalTracker() {
  ...
  const fetchGoals = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(await readErrorMessage(response, 'Failed to fetch goals'))
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(await readErrorMessage(response, `Failed to ${editingGoal ? 'update' : 'create'} goal`))
      }
  ...
  const handleDelete = async (goalId) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(await readErrorMessage(response, 'Failed to delete goal'))
      }

      showToast({ title: 'Goal deleted', description: 'The goal was removed successfully.', variant: 'info' })
      fetchGoals()
    } catch (err: any) {
      setError(err.message)
    } finally {
  ...
  const handleToggleComplete = async (goal) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(await readErrorMessage(response, 'Failed to update goal'))
      }
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4" data-lpignore="true">
              ...
              <label htmlFor="is_completed" className="inline-flex items-center gap-2 text-sm font-medium text-gray-700">
                <input
                  id="is_completed"
                  name="is_completed"
                  type="checkbox"
        ...
        {!loading && activeGoals.length > 0 && (
          <div className="space-y-3">
            {activeGoals.map((goal) => {
              ...
              return (
                <div key={goal.id} className="rounded-lg border border-gray-200 p-4 transition-colors hover:border-primary-300">
                        ...
                        {goal.target_date && (
                          <div className="flex items-center gap-1">
                            ...
                            <span>
                              ...
                              {daysLeft !== null && (
                                <span className={`ml-1 ${daysLeft < 0 ? 'text-red-600' : daysLeft <= 7 ? 'text-orange-600' : 'text-gray-500'}`}>
                ...
                <div key={goal.id} className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="ml-3 flex gap-1">
                      <Button type="button" variant="ghost" size="icon" onClick={() => handleEdit(goal)}>
                        <Edit size={16} />
                      </Button>
                      <Button
                        type="button"
