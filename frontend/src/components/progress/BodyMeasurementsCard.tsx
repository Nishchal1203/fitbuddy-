...
/* ─────────────────────────────────────────────
   TYPES
───────────────────────────────────────────── */
type MeasurementKey = 'chest' | 'waist' | 'hips' | 'biceps' | 'thighs' | 'shoulders'
...
const FALLBACK: MeasurementEntry[] = [
  { key: 'chest',     label: 'Chest',     current: 95,  previous: 97,  unit: 'cm', emoji: '' },
  { key: 'waist',     label: 'Waist',     current: 80,  previous: 83,  unit: 'cm', emoji: '' },
  { key: 'hips',      label: 'Hips',      current: 102, previous: 103, unit: 'cm', emoji: '' },
  { key: 'biceps',    label: 'Biceps',    current: 30,  previous: 29,  unit: 'cm', emoji: '' },
  { key: 'thighs',    label: 'Thighs',    current: 55,  previous: 56,  unit: 'cm', emoji: '' },
  { key: 'shoulders', label: 'Shoulders', current: 112, previous: 112, unit: 'cm', emoji: '' },
]
...
function MeasurementTile({ entry }: { entry: MeasurementEntry }) {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-brand-pale bg-white p-4 shadow-[0_2px_12px_-4px_#9567B920]">
      ...
      {/* current value */}
      <div>
        <span className="text-2xl font-bold text-brand-slate">{entry.current}</span>
        <span className="ml-1 text-sm font-medium text-brand-slate/50">{entry.unit}</span>
      </div>
