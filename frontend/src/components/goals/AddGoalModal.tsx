...
export default function AddGoalModal({ isOpen, onClose, onCreateGoal }: AddGoalModalProps) {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!Number.isFinite(current) || current < 0) nextErrors.currentValue = 'Current value must be 0 or higher.'
    if (!Number.isFinite(target) || target <= 0) nextErrors.targetValue = 'Target value must be greater than 0.'
      ...
      <form onSubmit={handleSubmit} className="space-y-4" data-lpignore="true">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-xl border border-brand-pale bg-brand-bg/60 p-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-brand-slate/60">Preview</p>
            <p className="mt-1 text-sm text-brand-slate">Estimated progress: <span className="font-bold">{computedProgress}%</span></p>
            <p className="text-xs text-brand-slate/60">Due: {toDueLabel(form.targetDate)}</p>
          </div>
        </div>
