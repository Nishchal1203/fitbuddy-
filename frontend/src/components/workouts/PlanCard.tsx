    ...
    <Card className="h-full transition-transform duration-200 hover:-translate-y-0.5 hover:shadow-md">
      <CardHeader className="items-start">
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {plan.isDraft ? (
              <Badge variant="custom">Draft</Badge>
            ) : (
              <Badge variant="upcoming">AI Recommended</Badge>
            )}
          </div>
        </div>
        {subscribed ? (
      ...
      <CardContent className="space-y-4">
        <p className="min-h-12 text-sm text-gray-600">
          {plan.description ||
            "A structured plan designed to keep you consistent."}
        </p>
      ...
      <CardFooter className="grid grid-cols-2 gap-2">
        ...
        <Button
          ...
          onClick={() => onToggleSubscribe(plan)}
        >
