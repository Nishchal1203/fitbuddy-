...
export default function CustomPlanBuilder({ isOpen, onClose, onCreateDraft }: CustomPlanBuilderProps) {
  ...
  const [exerciseOptions, setExerciseOptions] = useState<ExerciseOption[]>(FALLBACK_EXERCISE_OPTIONS)
  const [exerciseRows, setExerciseRows] = useState<DraftExerciseRow[]>([
    { key: 'row-1', exercise_id: null, sets: '3', reps: '10', rest: '60 sec', notes: '' }
  ])
  ...
  const resetAndClose = () => {
    ...
    setExerciseRows([{ key: 'row-1', exercise_id: null, sets: '3', reps: '10', rest: '60 sec', notes: '' }])
    onClose()
  }

  const updateExerciseRow = (key: string, patch: Partial<DraftExerciseRow>) => {
    setExerciseRows((prev) => prev.map((row) => (row.key === key ? { ...row, ...patch } : row)))
  }
  ...
  const addExerciseRow = () => {
    ...
    setExerciseRows((prev) => [
      ...prev,
      { key: `row-${nextIndex}-${Date.now()}`, exercise_id: null, sets: '3', reps: '10', rest: '60 sec', notes: '' }
    ])
  }

  const removeExerciseRow = (key: string) => {
    setExerciseRows((prev) => (prev.length > 1 ? prev.filter((row) => row.key !== key) : prev))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const selectedExercises: PlanExercise[] = exerciseRows
        ...
        .map((row) => {
          const selected = exerciseOptions.find((item) => item.id === row.exercise_id)
          return {
    ...
    <Modal isOpen={isOpen} onClose={resetAndClose} title="Create Your Own Plan">
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          >
            <p className="text-sm font-semibold text-gray-800">Manual Builder</p>
            <p className="mt-1 text-xs text-gray-500">Pick exercises, sets, reps, and rest yourself.</p>
          </button>
          ...
          >
            <div className="flex items-center gap-2">
              <Image src={aiIcon} alt="AI icon" width={18} height={18} className="h-[18px] w-[18px]" />
              <p className="text-sm font-semibold text-gray-800">Create Workout with AI</p>
            </div>
            <p className="mt-1 text-xs text-gray-500">Describe your goals and generate a plan from prompt.</p>
          </button>
        </div>
        ...
        {builderMode === 'ai' ? (
          <div className="space-y-3 rounded-lg border border-gray-200 p-3">
            <div className="flex items-center gap-2">
              <Image src={aiIcon} alt="AI icon" width={18} height={18} className="h-[18px] w-[18px]" />
              <p className="text-sm font-semibold text-gray-800">Define Your Goals for AI</p>
            </div>
            <Textarea
              name="ai_prompt"
              label="Prompt"
            ...
            <p className="text-xs text-gray-500">
              This is frontend-only for now. Backend AI generation will be connected later.
            </p>
          </div>
        ) : (
        <div className="space-y-3 rounded-lg border border-gray-200 p-3">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-gray-800">Plan Exercises</p>
            <Button type="button" variant="outline" onClick={addExerciseRow}>Add Exercise</Button>
          </div>

          {exerciseRows.map((row, index) => (
            <div key={row.key} className="rounded-md border border-gray-100 p-3">
              <div className="mb-2 flex items-center justify-between">
                <p className="text-xs font-semibold text-gray-500">Exercise {index + 1}</p>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <Select
                  ...
                  onChange={(e) => {
                    ...
                    updateExerciseRow(row.key, { exercise_id: value ? Number(value) : null })
                  }}
                  required
                />
                ...
                <Input
                  label="Rest"
                  value={row.rest}
                  onChange={(e) => updateExerciseRow(row.key, { rest: e.target.value })}
                ...
                <Input
                  label="Sets"
                  value={row.sets}
                  onChange={(e) => updateExerciseRow(row.key, { sets: e.target.value })}
                  placeholder="e.g. 3"
                />
                ...
                <Input
                  label="Reps"
                  value={row.reps}
                  onChange={(e) => updateExerciseRow(row.key, { reps: e.target.value })}
              ...
              <Input
                ...
                label="Notes"
                value={row.notes}
                onChange={(e) => updateExerciseRow(row.key, { notes: e.target.value })}
        ...
        <div className="rounded-lg border border-brand-gold/40 bg-brand-gold/10 p-3 text-sm text-yellow-800">
          <p className="inline-flex items-center gap-2 font-medium"><Sparkles size={14} /> {builderMode === 'ai' ? 'AI Plan Draft' : 'Custom Plan Builder'}</p>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit" loading={loading}>{builderMode === 'ai' ? 'Create with AI' : 'Save Plan'}</Button>
        </div>
      </form>
    </Modal>
  )
